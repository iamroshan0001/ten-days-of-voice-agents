...
def scene_text(scene_key: str, userdata: Userdata) -> str:
    """
    Build SHORT descriptive text for the current scene.
    NO choice listing - keep it brief for voice.
    """
    scene = WORLD.get(scene_key)
    if not scene:
        return "You're in a void. What do you do?"

    # Just the core description, no choice listing
    desc = scene['desc']
...
async def start_adventure(
    ...
    """Start a new adventure - SHORT intro."""
    ...
    # SHORT opening
    opening = (
        f"Welcome {userdata.player_name or 'traveler'} to Brinmere.\n\n"
...
async def player_action(
    ...
    """Accept player action, update state, return SHORT next scene."""
    userdata = ctx.userdata
    ...
    if not chosen_key:
        ...
        return "Try: 'inspect box', 'go to tower', or 'head to cottages'. What do you do?"
    ...
    # SHORT reply - just next scene description
...
async def show_journal(
    ...
    """Show SHORT journal summary."""
    userdata = ctx.userdata
    lines = []
    
    lines.append(f"Player: {userdata.player_name or 'Adventurer'}")
...
class GameMasterAgent(Agent):
    def __init__(self):
        instructions = """
        You are 'Aurek', the Game Master for a voice D&D adventure.
        
        CRITICAL RULES:
        - Keep responses SHORT (2-3 sentences max)
        - Don't list choices - players will figure it out
        - Always end with "What do you do?"
        - Be direct and concise
        
        Universe: Coastal fantasy village (Brinmere)
        Tone: Mysterious but brief
        
        Example GOOD response:
        "The hatch opens. Cold air rises from below. What do you do?"
        
        Example BAD response:
        "With the map's guidance, the ancient latch mechanism yields after a moment of careful manipulation, and the heavy iron hatch slowly creaks open, revealing a breath of cold, musty air rising from the darkness below. You can see rough stone steps spiraling downward..."
        """
        super().__init__(
